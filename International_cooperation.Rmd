...
breaks_distacne_fine <- c(-1, 0.1, 1000, 5000, 10000, 15000, 20000) # the minimum non-zero distance is 0.1725848 km
...
odd_ratio <- odd_ratio %>%
  ...
  mutate(Internet_Age = ifelse(Year < 1990, "Before", "After")) # the establishment of Postmodern Culture in 1990 as the first online-only journal
...
The animation shows two tendencies. First, the odds ratio of being cited is generally higher for remote collaboration than for on-site collaboration after 1990. Second, the variance in the odds ratio is higher for longer distances.

For the first tendency, the most likely explanation is that the internet has made it easier for researchers to collaborate remotely. I chose 1990 as the cut-off year because it is the year that the first online-only journal, Postmodern Culture, was established. For the data after 2015, there appears a downward trend in the odds ratio for all distances. This may be due to the data collection time, as the data is collected in 2020, and the papers published in 2015-2020 have not had enough time to be cited. For those high influence papers, it may take shorter time to be cited. It may be reasonable to use the citation as the quality of the paper. However, for the papers with relatively lower influence or some specific disciplines, it may take longer time to be cited and show their difference between those never cited papers. Therefore, the statistic power for distinguishing the difference between the citable and non-citable papers may significantly decrease as the existing time decreases. That would be the reason why the odds ratio is decreasing for the recent years.

For the second tendency, the variance in the odds ratio is higher for longer distances. This may be due to the fact that the sample size for longer distances is smaller, leading to a higher variance in the odds ratio. In this case, it implies that the higher likelihood of being cited for remote collaboration is stable and accurate.
...
The original paper found that remote collaboration has about a 0.85 relative probability of disruption compared to on-site collaboration without time effect. Our analysis shows about a 1.5 relative probability of being cited compared to on-site collaboration after 2005. However, the trade-off is not simply subtractive.
...
You may also find that the gif is not working in the pdf. This is because the pdf does not support gif. You can use the html output to see the gif.
